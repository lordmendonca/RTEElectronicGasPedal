...
#if (Number_of_ISRs > 0) && defined isr_testWDT__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_testWDT__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_testWDT;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_testWDT__INTC_NUMBER, (ISR_MIN_PRI - 6));
    CyIntSetPriority(isr_testWDT__INTC_NUMBER, ISR_SCALE(6));
    //\PF
	NVIC_INT_ENABLE(isr_testWDT__INTC_NUMBER);
#elif Number_of_ISRs > 0
#error  The Interrupt isr_testWDT could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_testWDT */

#if (Number_of_ISRs > 1) && defined isr_OSShutdown__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_OSShutdown__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_OSShutdown;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_OSShutdown__INTC_NUMBER, (ISR_MIN_PRI - 7));
    CyIntSetPriority(isr_OSShutdown__INTC_NUMBER, ISR_SCALE(7));
    //\PF
	NVIC_INT_ENABLE(isr_OSShutdown__INTC_NUMBER);
#elif Number_of_ISRs > 1
#error  The Interrupt isr_OSShutdown could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_OSShutdown */
